...
test('Patients Page UI Check', async ({ page }) => {
  ...
  // 2. Visual Comparison
  await expect(page).toHaveScreenshot('Patients-baseline.png', {
    maxDiffPixelRatio: 0.15, // High margin because patient data changes
    mask: [page.locator('.patient-row-id')], // Mask specific dynamic IDs if known
  });
  });